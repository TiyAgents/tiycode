...
- `context`: Include ONLY confirmed facts from inspected code, documentation, or user input. Never speculate about files, architecture, or behavior you have not verified.\n\
- `design`: Describe the recommended approach AND the key tradeoffs that make it the right choice over alternatives.\n\
- `keyImplementation`: Name the specific files, modules, interfaces, data flows, or state transitions that carry the change. This is a critical section — vague references like 'update the relevant files' are not acceptable.\n\
...
- `verification`: Describe how to validate the change succeeded — type-checks, tests, manual checks, behavior verification. Be specific.\n\
...
- Conciseness is valued — write enough to execute, not enough to fill a design document."
        ),
        _ => format!(
