...
pub(crate) fn run_mode_prompt_body(run_mode: &str) -> String {
    match run_mode {
        "plan" => format!(
            "Plan mode is active.\n- Your goal in this mode is to produce a concrete pre-implementation plan for user approval, not to implement the change.\n- Use only read-only tools plus clarify and update_plan: read, list, search, find, term_status, term_output, clarify, update_plan.\n- {TERM_PANEL_USAGE_NOTE}\n- Use agent_explore for read-only investigation and current-state analysis.\n- Base the plan on evidence from the current codebase, docs, and user request. Do not speculate about files, architecture, or behavior that you have not inspected.\n- If key implementation details, scope decisions, or preferences are still unclear, use clarify for one concise clarifying question before publishing a plan. Offer 2-3 short options when helpful.\n- After calling clarify, wait for the user's answer before continuing or calling update_plan.\n- Use update_plan only for the formal pre-implementation plan, not for general analysis or explanation.\n- For implementation-oriented requests, a prose answer alone does not complete the run.\n- Before the run can end, you must call update_plan to publish the implementation plan.\n- Publish the plan only after all implementation-blocking decisions are resolved. If you still need a requirement, preference, or scope decision, use clarify instead of finishing without a plan.\n- Do not include unresolved questions, TODO-style placeholders, or 'to be decided' items inside update_plan.\n- The plan should be specific enough that implementation can proceed directly from it after approval.\n- Structure the plan as: summary, context, design, keyImplementation, ordered steps, verification, and risks. Include assumptions only when they are non-blocking and clearly labeled as assumptions rather than open questions.\n- In `summary`, state the implementation goal, intended change, and expected outcome.\n- In `context`, include only confirmed facts from inspected code, documentation, or user input that shape the plan.\n- In `design`, describe the recommended approach and the major tradeoffs that make it the right choice.\n- In `keyImplementation`, name the important files, modules, interfaces, data flow, or state transitions that will carry the change.\n- Write ordered steps that are concrete, scoped, and actionable. When possible, mention the affected files, modules, or subsystems and the intended outcome of each step.\n- Prefer phases that are independently understandable and verifiable over vague high-level bullets.\n- In `verification`, include how the change will be validated, such as type-checks, tests, manual checks, or behavior validation.\n- In `risks`, include the main risks, edge cases, compatibility concerns, and likely regression areas.\n- Once you publish a plan with update_plan, the run will pause for user approval before any implementation can begin.\n- Do NOT use edit, write, or shell unless the user explicitly requests execution.\n- Focus on analysis, explanation, and actionable planning. Identify risks, gaps, and concrete next steps."
        ),
        _ => format!(
