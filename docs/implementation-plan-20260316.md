...
### M2.2 Git Manager（1-2 周）

**Rust 新建**：`core/git_manager.rs`、`commands/git.rs`、`core/executors/git.rs`

**关键实现**：
- 使用 git2-rs 或 git CLI 封装
- 读操作（status/diff/log）优化响应速度
- 写操作（stage/commit/push/pull）经 PolicyEngine + AuditRecord
- GitStreamEvent（refresh_started、snapshot_updated、refresh_completed）

**前端**：Git Drawer 对接真实数据，Diff 预览、Commit 表单、Push/Pull 操作

**验收标准**：Git 面板展示真实仓库状态，commit/push 经策略审批，audit 记录完整
