...
### M1.5 Built-In Agent Runtime（拆分为 4 个子阶段，总计 1-2 周）

**拆分原则**：

- 先打通最小可运行主链路，再补工具桥接与状态机
- helper 编排与 sidecar 清退单独收口，避免一个 milestone 同时承担过多变量
- 每个子阶段结束时，都应有单独 smoke test 或集成测试可验证

#### M1.5a Runtime 骨架与主 Run 流式链路（2-3 天）

**目标**：先建立 `AgentRunManager + BuiltInAgentRuntime + AgentSession`
最小骨架，打通不依赖真实工具执行的主 run 流式链路。

**Rust 新建/重构文件**：

- `core/agent_run_manager.rs` — parent run 创建、active run 约束、
  `effective_model_plan_json` 冻结
- `core/built_in_agent_runtime.rs` — runtime 创建、session 生命周期管理
- `core/agent_session.rs` — 单个 parent run 的 in-memory runtime session
- `commands/agent.rs` — 最小 `thread_start_run`
- `ipc/frontend_channels.rs` — 首版 `ThreadStreamEvent`（先覆盖 message/run lifecycle）
...
- 允许先用 mocked / noop `ToolGateway` 占位，优先打通 prompt -> streaming 文本回显

**验收标准**：

- 用户发送 prompt -> Rust runtime -> `tiy-core` -> LLM -> 流式回显线程
- 同一线程无法并发启动两个 active parent run
- `effective_model_plan_json` 在 run 创建时成功冻结并持久化

#### M1.5b Tool Executor Bridge、Approval 与 Cancellation（2-3 天）

**目标**：在主链路稳定后补齐 `ToolGateway` 桥接、审批挂起语义和取消状态机。

**Rust 新建/重构文件**：

- `core/agent_session.rs` — `set_tool_executor(...)` bridge、approval suspend、
  cancellation propagation
- `core/agent_run_manager.rs` — `WaitingApproval` / `Cancelling` / terminal status 收敛
- `commands/agent.rs` — `thread_cancel_run`、`tool_approval_respond`
- `ipc/frontend_channels.rs` — `tool_*`、`approval_required`、run status 事件
...
- approval 等待通过挂起 tool executor future 实现，parent run 在等待期间
  进入 `WaitingApproval`
- parent run 保留 `Cancelling` 中间态，用于等待 tool cleanup 和超时收尾
- 不引入 helper，先把 parent run 的状态机跑稳

**验收标准**：

- 工具调用可通过 `ToolGateway` adapter 返回 executed / approval required /
  denied / failed
- approval resolve 后原 tool executor future 能继续执行，run 不丢失上下文
- 取消中的 run 会先进入 `Cancelling`，再稳定落到 `Cancelled` 或 `Interrupted`

#### M1.5c Helper Orchestration 与 `run_helpers` 持久化（2-3 天）

**目标**：把 helper 能力作为 parent run 下的内部执行单元接入，并完成持久化模型切换。

**Rust 新建/重构文件**：

- `core/subagent/orchestrator.rs` — helper task 编排、取消传播、结果折叠与进度事件
- `core/subagent/runtime_orchestration.rs` — helper delegation tool 分类与 profile 定义
- `core/agent_session.rs` — helper 状态跟踪、helper 结果回注
- `persistence/repo/run_helper_repo.rs` — helper 摘要持久化
- `src-tauri/migrations/<timestamp>_run_helpers.sql` — `run_subtasks -> run_helpers`
  迁移与兼容处理
- `ipc/frontend_channels.rs` — `subagent_*` 与进度事件
...
- helper 继承 parent run mode / tool ceiling，不能绕过 `plan` 模式只读限制

**验收标准**：

- helper 状态可折叠展示在主线程
- `run_helpers` 持久化生效，旧 `run_subtasks` 不再作为新链路写入目标
- helper 取消、失败、升级审批都能正确折叠回 parent run

#### M1.5d Sidecar 清退与前端最小切换（1-2 天）

**目标**：移除 sidecar 残留路径，让线程运行默认完全切到 built-in runtime；
前端只完成 M1.5 所需的最小闭环切换，其余全面收口仍留给 M1.7。
...
- 主线程支持 helper 折叠块渲染
- 保留 M1.7 再统一处理 mock fixtures、localStorage fallback 和全量 adapter 收口

**验收标准**：

- 线程运行链路不再依赖 sidecar 进程或 sidecar bridge 类型
- PromptInput 到线程流式回显的最小闭环已切换到 Rust runtime
- app 启动路径中不再包含 sidecar 启动逻辑
...
M1.1 基础设施
 ...
 │         └── M1.4 Thread
 │              └── M1.5a Runtime 骨架
 │                   └── M1.5b Tool/Approval/Cancel
 │                        └── M1.5c Helper Orchestration
 │                             └── M1.5d Sidecar 清退
...
- **Runtime 与工具层并行**：
  M1.5a-M1.5b 期间可以先用 mocked ToolGateway 事件打通 `tiy-core`
  streaming 渲染与 approval 状态机
- **M1.2 与 M1.8 可并行**：
  Index 仅依赖 M1.1
- **M2.1 与 M2.2 可并行**：
...
| M1.5a 完成后 | 烟测：prompt -> Rust runtime -> `tiy-core` -> streaming response（无 helper、可用 mocked ToolGateway） |
| M1.5b 完成后 | 集成测试：tool executor suspend/resume、approval resolve、`Cancelling -> Cancelled/Interrupted` |
| M1.5c 完成后 | 集成测试：helper 折叠展示、`run_helpers` 持久化、helper escalation / cancellation |
| M1.5d 完成后 | 启动与线程链路 smoke test：无 sidecar 启动逻辑，PromptInput 已走 built-in runtime |
