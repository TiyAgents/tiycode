...
### 三、Plan mode 下的结构化计划约束

当前 plan 模式已经具备“需要先分析、必要时先 clarify、结束前必须 update_plan、发布后等待批准”的基本流程约束，但对“计划本身应该怎么写”仍然不够具体。建议在 plan mode 的 Prompt 中进一步强化以下要求：

1. Plan mode 的目标是产出待用户批准的实施方案，而不是只给分析结论，也不是直接实现改动。
2. 计划必须基于已经读取到的代码、文档和用户请求编写，不应臆测尚未检查过的文件位置、架构细节或行为现状。
3. 如果仍存在阻塞实现的需求、范围或偏好决策，应先使用 `clarify` 消除分歧，而不是把未决问题写进计划。
4. 发布的 `update_plan` 不应包含 unresolved questions、TODO 占位或 “to be decided” 一类内容。
5. 计划应当足够具体，使后续执行阶段在获批后可以直接按计划推进，而不需要重新解释意图。
6. `update_plan` 应优先采用结构化写法，至少包含 `summary`、有顺序的 `steps`、`verification` 与 `risks`。
7. 每个步骤都应是具体、可执行、可验证的动作，粒度适中，并在可能时指出受影响的文件、模块或子系统以及预期产出。
8. 计划应优先按可独立理解和验证的阶段组织，而不是按思考过程罗列宽泛的高层口号。
9. 计划中应明确最终如何验证改动，包括 type-check、测试、手动检查或关键行为验证方式。
10. 计划中应显式列出主要风险、边界情况、兼容性顾虑和潜在回归点，使用户在批准前就能判断实施成本与风险。
